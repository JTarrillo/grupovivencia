...
<body data-new-gr-c-s-check-loaded="14.1042.0" data-gr-ext-installed="">
    ...
    <!-- DEBUG JSON LOTES DISPONIBLES -->
    <div id="debug_lots_json" style="margin: 20px 0; font-size: 0.95rem;"></div>
                                            ...
                                            <div class="card-body">
                                                ...
                                                <div class="text-center mt-3">
                                                    <button class="btn btn-warning"
                                                        onclick="ReserveLot.openModal()">Reservar
                                                        Lote</button>
                                                </div>
                                            </div>
                                        </div>
    ...
    <!-- Modal para reservar lote -->
    <div class="modal fade" id="modalReserveLot" tabindex="-1" role="dialog" aria-labelledby="modalReserveLotLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalReserveLotLabel">Reservar Lote</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="formReserveLot">
                        <div class="form-group">
                            <label for="reserveAbono">Monto de abono (S/)</label>
                            <input type="number" class="form-control" id="reserveAbono" name="abono" min="1000"
                                required>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-warning" onclick="ReserveLot.submit()">Reservar</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
    // Clase modular para reserva de lote
    const ReserveLot = {
        openModal: function() {
            // Cierra cualquier popup de SweetAlert2
            if (typeof Swal !== 'undefined' && Swal.close) {
                Swal.close();
            }
            // Cierra el modal de contrato si está abierto
            $('#newContractModal').modal('hide');
            $('#contractPreviewModal').modal('hide');
            $('#editContractModal').modal('hide');
            setTimeout(function() {
                $('#modalReserveLot').modal('show');
            }, 300);
        },
        submit: function() {
            const abono = document.getElementById('reserveAbono').value;
            if (!abono || abono < 1000) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Monto inválido',
                    text: 'El abono debe ser mayor a S/ 1000.'
                });
                return;
            }
            if (!selectedLot || !selectedCustomer) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Faltan datos',
                    text: 'Debes seleccionar cliente y lote antes de reservar.'
                });
                return;
            }
            // Enviar reserva al backend
            fetch('/dashboard/inmueble/reserve_lot', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
                    },
                    body: `lot_id=${selectedLot.id}&amount=${abono}&customer_id=${selectedCustomer.id}`
                })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        $('#modalReserveLot').modal('hide');
                        Swal.fire({
                            icon: 'success',
                            title: 'Lote reservado',
                            text: data.message || 'Reserva exitosa.'
                        });
                        // Opcional: recargar lotes disponibles
                        if (typeof loadAvailableLots === 'function') {
                            loadAvailableLots(); // Usar solo la función del JS externo
                        } else {
                            console.error('No se encontró la función loadAvailableLots en el JS externo.');
                        }
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: data.message || 'No se pudo reservar el lote.'
                        });
                    }
                })
                .catch(error => {
    ...
    function displayAvailableLots(lots) {
        ...
        lots.forEach(lot => {
               ...
               `;
                    ...
                    // Cierra el modal de reserva si está abierto antes de mostrar el error
                    if ($('#modalReserveLot').is(':visible')) {
                        $('#modalReserveLot').modal('hide');
                    }
                }
                ...
                function selectLot(lot) {
                    ...
                    document.getElementById('selected_lot_price').textContent = parseFloat(lot.current_price || 0)
                        .toLocaleString(
                            'es-PE');
                ...
                function filterAvailableLots() {
                    ...
                    if (projectId) {
                        const filteredLots = window.availableLots.filter(lot => lot.project_id == projectId);
                ...
                function calculateContract() {
                    ...
                    if (financedAmount > 0 && monthlyRate > 0) {
                        monthlyPayment = financedAmount * (monthlyRate * Math.pow(1 + monthlyRate, months)) / (Math.pow(
                            1 +
                    ...
                    document.getElementById('sim_monthly_payment').textContent = monthlyPayment.toLocaleString(
                        'es-PE', {
                ...
                function updateSummary() {
                    ...
                    if (selectedLot) {
                        ...
                        document.getElementById('summary_lot_price_final').textContent = parseFloat(selectedLot
                                .current_price || 0)
                    ...
                    document.getElementById('summary_down_payment_final').textContent = downPayment.toLocaleString(
                        'es-PE');
                    ...
                    document.getElementById('summary_start_date').textContent = contractDate.toLocaleDateString(
                        'es-PE');
                    ...
                    document.getElementById('summary_total_interest').textContent = totalInterest.toLocaleString(
                        'es-PE');
