...
<body data-new-gr-c-s-check-loaded="14.1042.0" data-gr-ext-installed="">
    ...
    function editLot(lotId) {
        // Fetch lot data
        fetch('/dashboard/inmueble/api/get_lot/' + lotId)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    currentLotData = data.lot;
    ...
    function loadLotDataToModal(lot) {
        ...
        // Update price per sqm from project
        updateLotPricePerSqm();

        // Show customer info if lot is sold/reserved
    ...
    function calculateLotPrice() {
        ...
        if (area > 0 && pricePerSqm > 0) {
            ...
            document.getElementById('calc_initial').textContent = 'S/ ' + initialPayment.toLocaleString('es-PE') + ' ' +
                initialLabel;
    ...
    function view_lot(lotId) {
        // Fetch detailed lot data
        fetch('/dashboard/inmueble/api/get_lot_details/' + lotId)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    currentLotData = data.lot;
                    loadLotDetailsToModal(data.lot, data.project, data.customer, data.contract);
    ...
    function loadLotDetailsToModal(lot, project, customer = null, contract = null) {
        // Información básica
        ...
        document.getElementById('view_area').textContent = parseFloat(lot.area_sqm || 0).toLocaleString('es-PE', {
        ...
        document.getElementById('view_price_per_sqm').textContent = 'S/ ' + pricePerSqm.toLocaleString('es-PE', {
            minimumFractionDigits: 2
        });
        document.getElementById('view_total_lot_price').textContent = 'S/ ' + totalLotPrice.toLocaleString('es-PE', {
            minimumFractionDigits: 2
        });
        ...
        // Obtener meses de financiamiento desde el proyecto
        let months = 36;
        ...
        const monthlyPayment = months > 0 ? financeAmount * monthlyRate * Math.pow(1 + monthlyRate, months) / (Math.pow(
            1 + monthlyRate, months) - 1) : 0;
        const appreciation = currentPrice - basePrice;
        // Actualizar label de meses
        document.getElementById('monthly_payment_label').textContent = `Cuota Mensual Aprox. (${months} meses)`;

        document.getElementById('view_initial_payment').textContent = 'S/ ' + initialPayment.toLocaleString('es-PE') +
            ' ' + initialLabel;
        ...
        document.getElementById('view_price_update_date').textContent = new Date(lot.price_last_updated || lot
            .updated_at).toLocaleDateString('es-PE');
        ...
        document.getElementById('view_last_updated').textContent = 'Actualizado: ' + new Date(lot.updated_at)
            .toLocaleDateString('es-PE');

        // Título del modal
        document.getElementById('viewLotModalLabel').textContent =
            `Detalles del Lote ${lot.lot_number} - ${project?.name || 'N/A'}`;
    }
