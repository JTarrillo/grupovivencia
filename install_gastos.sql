...
CREATE TABLE IF NOT EXISTS `gasto_subcategorias` (
  ...
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  FOREIGN KEY (`gasto_tipo_id`) REFERENCES `gasto_tipos`(`id`) ON DELETE CASCADE ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `compra_documentos` (
  ...
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  FOREIGN KEY (`compra_id`) REFERENCES `compras`(`id`) ON DELETE CASCADE ON UPDATE RESTRICT,
...
-- 6. ACTUALIZAR TABLA COMPRAS (si faltan campos)
...
-- Agregar índices (no causarán problemas si ya existen)
